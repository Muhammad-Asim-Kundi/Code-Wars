...
function revrot(str, sz) {
    ...
    // Break the string into chunks of size sz
    for(let i = 0; i + sz <= str.length; i += sz) {
      let chunk = str.slice(i, i + sz);
      let sum = 0;
  
      // Calculate the sum of digits in the chunk
      for(let digit of chunk) {
        sum += parseInt(digit);
      }
     // If the sum of digits is divisible by 2, reverse the chunk else Rotate the chunk to the left by one position
      sum % 2 === 0 ? result += chunk.split('').reverse().join('') : result += chunk.slice(1) + chunk[0]
